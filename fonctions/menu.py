...
from fonctions.journal_de_guerre_de_guildes.fonctions_journal_guerre import analyser_journaux_guerre_guildes
...
def afficher_aide(logger=None):
    ...
    message = (
        ...
        "F7 : Analyser les journaux de guerre de guildes\n"
...
def boucle_principale(logger, window, overlay: Overlay):
    ...
    logger.info(
        "⌨️  F1 aide | F3 capture | F6 debug | F7 analyse journaux | F12 recapture | F9 debug log | ESC quitter"
    )
    ...
    keyboard.add_hotkey('f7', lambda: analyser_journaux_guerre_guildes(logger, window, overlay))
